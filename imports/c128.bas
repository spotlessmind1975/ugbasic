...
'  ****************************************************************************/
'  ****************************************************************************
'  *** IMPORT DECLARES FOR TARGET c128 (Commodore 128 with CPU 8502)
'  ****************************************************************************
...
' [NOT WORKING]
DECLARE SYSTEM PROC cint AT $FF81 ON C128
...
' program cartridge.
DECLARE SYSTEM PROC ioinit AT $FF84 ON C128
...
DECLARE SYSTEM PROC ramtas AT $FF87 ON C128
...
DECLARE SYSTEM PROC restor AT $FF8A ON C128
...
DECLARE SYSTEM PROC vectorRead AT $FF8D( buffer AS ADDRESS ON REG(YX), #1 ON REG(CARRY) ) ON C128
DECLARE SYSTEM PROC vectorWrite AT $FF8D( buffer AS ADDRESS ON REG(YX), #0 ON REG(CARRY) ) ON C128
...
DECLARE SYSTEM PROC setmsg AT $FF90( bitmask AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC second AT $FF93( address AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC tksa AT $FF96( command AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC memtopWrite AT $FF99( address AS ADDRESS ON REG(YX), #0 ON REG(CARRY) ) ON C128
DECLARE SYSTEM FUNCTION memtopRead AT $FF99( #1 ON REG(CARRY) ) RETURN REG(YX) AS ADDRESS ON C128
...
DECLARE SYSTEM PROC membotWrite AT $FF99( address AS ADDRESS ON REG(YX), #0 ON REG(CARRY) ) ON C128
DECLARE SYSTEM FUNCTION membotRead AT $FF99( #1 ON REG(CARRY) ) RETURN REG(YX) AS ADDRESS ON C128
...
DECLARE SYSTEM PROC scnkey AT $FF9F ON C128
...
DECLARE SYSTEM PROC settmo AT $FFA2( enable AS BYTE ON REG(A) ) ON C128
DECLARE SYSTEM PROC settmoEnable AT $FFA2( #settmoEnable ON REG(A) ) ON C128
DECLARE SYSTEM PROC settmoDisable AT $FFA2( #settmoDisable ON REG(A) ) ON C128
...
DECLARE SYSTEM FUNCTION acptr AT $FFA5 RETURN REG(A) AS BYTE ON C128
...
' the device.
DECLARE SYSTEM PROC ciout AT $FFA8 ( data AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC untalk AT $FFAB ON C128
...
DECLARE SYSTEM PROC unlsn AT $FFAE ON C128
...
' information.
DECLARE SYSTEM PROC listen AT $FFB1 ( device AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC talk AT $FFB4 ( device AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM FUNCTION readst AT $FFB7 RETURN REG(A) AS BYTE ON C128
...
DECLARE SYSTEM PROC setlfs AT $FFBA ( address AS BYTE ON REG(A), device AS BYTE ON REG(X), command AS BYTE ON REG(Y) ) ON C128
...
DECLARE SYSTEM PROC setnam AT $FFBD ( len AS BYTE ON REG(A), filename AS ADDRESS ON REG(YX) ) ON C128
...
DECLARE SYSTEM PROC open AT $FFC0 ON C128
...
DECLARE SYSTEM PROC close AT $FFC3 ( file AS BYTE ON REG(A) ) ON C128
...
'
DECLARE SYSTEM PROC chkin AT $FFC6 ( file AS BYTE ON REG(X) ) ON C128
...
DECLARE SYSTEM PROC chkout AT $FFC9 ( file AS BYTE ON REG(X) ) ON C128
...
' routine is executed.
DECLARE SYSTEM PROC clrchn AT $FFCC ON C128
...
DECLARE SYSTEM FUNCTION chrin AT $FFCF RETURN REG(A) AS BYTE ON C128
...
DECLARE SYSTEM PROC chrout AT $FFD2( char AS BYTE ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC load AT $FFD5( mode AS BYTE ON REG(A), address AS ADDRESS ON REG(YX) ) ON C128
...
' is stopped.
DECLARE SYSTEM PROC save AT $FFD8( startAddress AS ADDRESS ON REG(34,35), endAddress AS ADDRESS ON REG(YX), #34 ON REG(A) ) ON C128
...
DECLARE SYSTEM PROC settim AT $FFDB( jiffies AS DWORD ON REG(AXY) ) ON C128
...
' significant byte.
DECLARE SYSTEM FUNCTION rdtim AT $FFDE RETURN REG(AXY) AS DWORD ON C128
...
DECLARE SYSTEM FUNCTION stop AT $FFE1 RETURN REG(ZERO) AS BYTE ON C128
...
DECLARE SYSTEM FUNCTION getin AT $FFE4 RETURN REG(A) AS BYTE ON C128
...
DECLARE SYSTEM PROC clall AT $FFE7 ON C128
...
DECLARE SYSTEM PROC udtim AT $FFEA ON C128
...
DECLARE SYSTEM FUNCTION screenX AT $FFED RETURN REG(X) AS BYTE ON C128
DECLARE SYSTEM FUNCTION screenY AT $FFED RETURN REG(Y) AS BYTE ON C128
...
' registers.
DECLARE SYSTEM PROC plot AT $FFF0( x AS BYTE ON REG(Y), y AS BYTE ON REG(X), #0 ON REG(CARRY) ) ON C128
DECLARE SYSTEM FUNCTION plotX AT $FFF0( #1 ON REG(CARRY) ) RETURN REG(Y) AS BYTE ON C128
DECLARE SYSTEM FUNCTION plotY AT $FFF0( #1 ON REG(CARRY) ) RETURN REG(X) AS BYTE ON C128
...
DECLARE SYSTEM FUNCTION iobase AT $FFF3 RETURN REG(YX) AS ADDRESS ON C128
