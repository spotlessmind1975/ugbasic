            ...
            switch( variable->type ) {
                ...
                case VT_COMPILED_IMAGES:
                    if ( variable->bankAssigned != -1 ) {
                        ...
                        for( int i=0; i<variable->memoryOffsetCount; ++i ) {
                            outline2("JMP *+$%4.4x+3*%d", (variable->memoryOffsetCount-i), (variable->memoryOffsetCount-i) );
                        }
                        if ( ! variable->absoluteAddress ) {
                            if ( variable->valueBuffer ) {
                                out0("   fcb ");
...
void variable_cleanup( Environment * _environment ) {
    ...
    deploy_inplace_preferred( dcommon, src_hw_coco3b_dcommon_asm);
    deploy_inplace_preferred( dload, src_hw_coco3b_dload_asm);
    deploy_inplace_preferred( dsave, src_hw_coco3b_dsave_asm);
