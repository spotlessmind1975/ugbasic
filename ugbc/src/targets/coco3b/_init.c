...
void target_initialization( Environment * _environment ) {
    ...
    deploy( vars, src_hw_coco3b_vars_asm);
    deploy_deferred( startup, src_hw_coco3b_startup_asm);
    bank_define( _environment, "STRINGS", BT_STRINGS, 0x4200, NULL );

    outline0( "JSR COCO3BSTARTUP2");
    outline0( "JSR COCO3BSTARTUP" );

    setup_text_variables( _environment );

    gime_initialization( _environment );
    cpu_call( _environment, "COCO3BAUDIO1STARTUP" );
