...
void target_linkage( Environment * _environment ) {
    ...
    BUILD_TOOLCHAIN_CC65_EXEC( _environment, "c16", executableName, labelsFileName, listingFileName, "-u __EXEHDR__" );
     ...
     if ( _environment->listingFileName ) {
        ...
        if ( _environment->profileFileName && _environment->profileCycles ) {
            ...
            sprintf( commandLine, "\"%s\" -X 0000 -R 100d -l 0fff \"%s\" -u \"%s\" -p \"%s\" %d",
                executableName,
