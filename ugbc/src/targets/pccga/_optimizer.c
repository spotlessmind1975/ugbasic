...
static void basic_peephole(POBuffer buf[LOOK_AHEAD], int zA, int zB) {
	...
	if( 
		...
		optim( buf[0], RULE "(CALL+RET)->(JP)", "\tJMP %s", v1->str );
		optim( buf[1], NULL, NULL );
    }
...
//                 if ( po_buf_match( buf[0], " ; VSP" ) ) {
//                     done = 1;
//                 } else if( ( ( po_buf_match( buf[0], " LD A, *", v2 ) && po_buf_match( buf[1], " LD (*), A", v2 ) ) ||
//                              ( po_buf_match( buf[0], " LD HL, *", v2 ) && po_buf_match( buf[1], " LD (*), HL", v2 ) ) )
//                             ) 
//                               {
...
//                     ++_environment->removedAssemblyLines;
//                     ++_environment->removedAssemblyLines;
//                 } else if( ( po_buf_match( buf[0], " LD (*), A", v2 ) ) ||
//                              po_buf_match( buf[0], " LD (*), HL", v2 ) ) {
//                     char * realVarName = strdup( v2->str );
//                     char * c = strstr( realVarName, "+" );
//                     if ( c ) {
//                         *c = 0;
    ...
    while( !feof(fileAsm) ) {
        ...
        if ( po_buf_match( bufLine, " ; V *", v1 ) ) {
            ...
            while( !feof(fileAsm) ) {
                ...
                if ( ! result ) result = po_buf_match(bufLine, " ADD (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " AND (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " BIT (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " CP (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " CPL (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " LD *, (*)", v2, v1 );
                if ( ! result ) result = po_buf_match(bufLine, " OR (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " SBC (*)", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " XOR (*)", v1 );

                if ( ! result ) result = po_buf_match(bufLine, " ADC *", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " ADD *", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " AND *", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " BIT *", v1 );
                if ( ! result ) result = po_buf_match(bufLine, " CP *", v1 );
            ...
            while( !feof(fileAsm) ) {
                ...
                if( ( ( po_buf_match( buf[0], " LD A, *", v2 ) && po_buf_match( buf[1], " LD (*), A", v2 ) ) ||
                             ( po_buf_match( buf[0], " LD HL, *", v2 ) && po_buf_match( buf[1], " LD (*), HL", v2 ) ) )
                            ) 
                              {
                    char * realVarName = strdup( v2->str );
