...
void blit_image( Environment * _environment, char * _blit, char * _x, char * _y, char * _frame, char * _sequence, int _flags ) {
    ...
    switch( vtImage ) {
        case VT_SEQUENCE:
            if ( !sequence ) {
                if ( !frame ) {
                    vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, "", "", image->frameSize, image->frameCount, _flags );
                } else {
                    vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, frame->realName, "", image->frameSize, image->frameCount, _flags );
                }
            } else {
                if ( !frame ) {
                    vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, "", sequence->realName, image->frameSize, image->frameCount, _flags );
                } else {
                    vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, frame->realName, sequence->realName, image->frameSize, image->frameCount, _flags );
                }
            }
            break;
        case VT_IMAGES:
            if ( !frame ) {
                vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, "", NULL, image->frameSize, 0, _flags );
            } else {
                vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, frame->realName, NULL, image->frameSize, 0, _flags );
            }
            break;
        case VT_IMAGE:
            vic2_blit_image( _environment, sources, _environment->blit.sourceCount, blitLabel, x->realName, y->realName, NULL, NULL, 0, 0, _flags );
            break;
        default:
