...
Generally speaking, it is not advisable to do a ''PUT IMAGE'' of a ''NEW SEQUENCE'' that has not 
...
In generale, non è consigliabile eseguire un ''PUT IMAGE'' di una ''NEW SEQUENCE'' che 
non sia stata inizializzata, almeno una volta, da un ''GET IMAGE''. Infatti, il 
''PUT IMAGE'' tenta di disegnare il contenuto di un frame non inizializzato (ad esempio: 
tutti zeri), inclusa la palette. Quindi potrebbe disegnare tutto vuoto in termini di 
bitmap e quindi sovrascrivere l'intera palette con zeri, con un risultato inaspettato. 
Quindi devi usare un ''GET IMAGE'' preliminare su qualsiasi ''NEW SEQUENCE''.
