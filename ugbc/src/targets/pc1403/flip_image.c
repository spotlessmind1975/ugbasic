    ...
    switch( resource->type ) {
        case VT_SEQUENCE:
            if ( image->bankAssigned != -1 ) {
                ...
                // // tms9918_flip_image( _environment, &resource, NULL, NULL, image->frameSize, 0, _direction );
                ...
                if ( !sequence ) {
                    if ( !frame ) {
                        // // tms9918_flip_image( _environment, resource, "", "", image->frameSize, image->frameCount, _direction );
                    } else {
                        // // tms9918_flip_image( _environment, resource, frame->realName, "", image->frameSize, image->frameCount, _direction );
                    }
                } else {
                    if ( !frame ) {
                        // // tms9918_flip_image( _environment, resource, "", sequence->realName, image->frameSize, image->frameCount, _direction );
                    } else {
                        // // tms9918_flip_image( _environment, resource, frame->realName, sequence->realName, image->frameSize, image->frameCount, _direction );
                    }
                }
            }
            break;
        case VT_IMAGES:
            if ( image->bankAssigned != -1 ) {
                ...
                if ( !frame ) {
                    // // tms9918_flip_image( _environment, resource, "", NULL, image->frameSize, 0, _direction );
                } else {
                    // // tms9918_flip_image( _environment, resource, frame->realName, NULL, image->frameSize, 0, _direction );
                }
            }
            break;
        case VT_IMAGE:
        case VT_TARRAY:
            if ( image->residentAssigned ) {
                ...
                // // tms9918_flip_image( _environment, &resource, NULL, NULL, 0, 0, _direction );
