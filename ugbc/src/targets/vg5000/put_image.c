    ...
    switch( image->type ) {
        case VT_SEQUENCE:
            if ( image->residentAssigned ) {
                ...
                if ( !sequence ) {
                    if ( !frame ) {
                        ef9345_put_image( _environment, image->realName, x->realName, y->realName, "", "", image->frameSize, image->frameCount, _flags );
                    } else {
                        ef9345_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, "", image->frameSize, image->frameCount, _flags );
                    }
                } else {
                    if ( !frame ) {
                        ef9345_put_image( _environment, image->realName, x->realName, y->realName, "", sequence->realName, image->frameSize, image->frameCount, _flags );
                    } else {
                        ef9345_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, sequence->realName, image->frameSize, image->frameCount, _flags );
                    }
                }
            } else {
                if ( !sequence ) {
        ...
        case VT_IMAGES:
            if ( image->residentAssigned ) {
                ...
                if ( !frame ) {
                    ef9345_put_image( _environment, bankWindowName, x->realName, y->realName, "", NULL, image->frameSize, 0, _flags );
                } else {
                    ef9345_put_image( _environment, bankWindowName, x->realName, y->realName, frame->realName, NULL, image->frameSize, 0, _flags );
                }
            } else {
                if ( !frame ) {
                    ef9345_put_image( _environment, image->realName, x->realName, y->realName, "", NULL, image->frameSize, 0, _flags );
                } else {
                    ef9345_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, NULL, image->frameSize, 0, _flags );
                }
            }
            break;
        case VT_IMAGE:
            if ( image->residentAssigned ) {
                ...
                ef9345_put_image( _environment, bankWindowName, x->realName, y->realName, NULL, NULL, 0, 0, _flags );
            } else {
                ef9345_put_image( _environment, image->realName, x->realName, y->realName, NULL, NULL, 0, 0, _flags );
            }
            break;
        default:
