...
static void basic_peephole(Environment * _environment, POBuffer buf[LOOK_AHEAD], int zA, int zB) {
    ...
    if ( po_buf_match( buf[0], " ST* *", v1, v2 )
    &&   po_buf_match( buf[1], " CLR*", v3 )
    &&   po_buf_match( buf[2], " LD* *", v4, v5 )
    &&   po_buf_strcmp(v1, v4)==0
    &&   po_buf_strcmp(v2, v5)==0
    &&   po_buf_strcmp(v1, v3)!=0
    ) {
        optim( buf[2], RULE "(ST*, CLR, LD*)->(STD*, CLR)", NULL);
        ++_environment->removedAssemblyLines;
    }
    ...
    && ( (strcmp( v1->str, "D" ) == 0) && (strcmp( v1->str, "X" ) == 0) && (strcmp( v1->str, "Y" ) == 0) )
    && po_buf_strcmp(v1, v2)==0
     ) {
        optim(buf[1], RULE "(LOAD/STORE(8 bit),CMP#0)->(LOAD/STORE)", NULL);
        ++_environment->removedAssemblyLines;
    }
    ...
    && (po_buf_match(buf[2], " BLT *", v4) == NULL) 
    && (po_buf_match(buf[2], " BGT *", v4) == NULL) 
    && (po_buf_match(buf[2], " BGE *", v4) == NULL) 
    && (po_buf_match(buf[2], " BHI *", v4) == NULL) 
    && (po_buf_match(buf[2], " BHS *", v4) == NULL) 
    && po_buf_strcmp(v1, v2)==0
     ) {
        optim(buf[1], RULE "(LOAD/STORE(16 bit),CMP#0,!check carry)->(LOAD/STORE)", NULL);
        ++_environment->removedAssemblyLines;
    }
    ...
    && po_buf_strcmp(v1, v2)==0 ) {
        optim(buf[1], RULE "(STDa,LDDa,STDb)->(STDb)", NULL );
        ++_environment->removedAssemblyLines;
    }
    ...
    if( po_buf_match(buf[0], " CLRD") && po_buf_match(buf[1], " LDD *", v1 ) ) {
        optim(buf[0], RULE "(CLRS,LDD)->(LDD)", NULL );
    }

    if( po_buf_match(buf[0], " LDD #0000") || po_buf_match(buf[0], " LDD #$0000") ) {
        optim(buf[0], RULE "(LDD#0000)->(CLRA, CLRB)", "\tCLRD" );
    }
    ...
    &&  ( strcmp(v5->str, "ANDB" ) != 0 && strcmp(v5->str, "ORB" ) != 0 && strcmp(v5->str, "EORB" ) != 0 )
        ) {
        ...
        if ( po_buf_match( bufLine, " ; V *", v1 ) ) {
            ...
            while( !feof(fileAsm) ) {
                ...
                if ( 
                    ...
                    UnusedSymbol * tmp3 = currentlySymbols;
                    while( tmp3 ) {
                        if ( strcmp( realVarName2, tmp3->realName ) == 0 ) {
                            break;
                        }
                        tmp3 = tmp3->next;
                    }
                    ...
                    ( ( po_buf_match( buf[0], " LDA #*", v1 ) || po_buf_match( buf[0], " CLRA") ) && po_buf_match( buf[1], " STA *", v2 ) && po_buf_match( buf[4], " ABX" ) ) ||
                    ( ( po_buf_match( buf[0], " LDB #*", v1 ) || po_buf_match( buf[0], " CLRB") ) && po_buf_match( buf[1], " STB *", v2 ) && po_buf_match( buf[4], " ABX" )  ) 
                    ) {
                    ...
                    if ( tmp ) {
                        ...
                        po_buf_match( buf[1], " LDD *", v4 ) && 
                        po_buf_match( buf[2], " ST* *", v5, v6 ) && 
                        po_buf_cmp( v2, v4 ) == 0 &&
                        ( strcmp( v5->str, "A" ) == 0 || strcmp( v5->str, "B" ) == 0 )
                    ) {
                    ...
                    if ( tmp && ( tmp2 || tmp3 ) ) {
                        ...
                        // printf(" APPLIED #1\n");
                        // optim( buf[0], RULE "unused temporary", NULL );
                        optim( buf[0], RULE "unused temporary", "\tST%s %s", v5->str, v6->str );
                        optim( buf[1], RULE "unused temporary", NULL );
                        optim( buf[2], RULE "unused temporary", NULL );
                        ++_environment->removedAssemblyLines;
                        ++_environment->removedAssemblyLines;
