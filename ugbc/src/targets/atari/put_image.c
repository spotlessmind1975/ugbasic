...
void put_image( Environment * _environment, char * _image, char * _x, char * _y, char * _frame ) {
    ...
    switch( image->type ) {
        case VT_IMAGES:
            if ( !frame ) {
                gtia_put_image( _environment, image->realName, x->realName, y->realName, "", image->frameSize );
            } else {
                gtia_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, image->frameSize );
            }
            break;
        case VT_IMAGE:
            gtia_put_image( _environment, image->realName, x->realName, y->realName, NULL, 0 );
            break;
        default:
