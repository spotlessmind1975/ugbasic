...
void flip_image_vars( Environment * _environment, char * _image, char * _frame, char * _sequence, char * _direction ) {
    ...
    switch( resource->type ) {
        case VT_SEQUENCE:
            if ( !sequence ) {
                if ( !frame ) {
                    gtia_flip_image( _environment, resource, "", "", image->frameSize, image->frameCount, _direction );
                } else {
                    gtia_flip_image( _environment, resource, "", frame->realName, image->frameSize, image->frameCount, _direction );
                }
            } else {
                if ( !frame ) {
                    gtia_flip_image( _environment, resource, "", sequence->realName, image->frameSize, image->frameCount, _direction );
                } else {
                    gtia_flip_image( _environment, resource, frame->realName, sequence->realName, image->frameSize, image->frameCount, _direction );
                }
            }
            break;
        case VT_IMAGES:
            if ( !frame ) {
                gtia_flip_image( _environment, resource, "", NULL, image->frameSize, 0, _direction );
            } else {
                gtia_flip_image( _environment, resource, frame->realName, NULL, image->frameSize, 0, _direction );
            }
            break;
        case VT_IMAGE:
        case VT_ARRAY:
            gtia_flip_image( _environment, resource, NULL, NULL, 0, 0, _direction );
            break;
        default:
