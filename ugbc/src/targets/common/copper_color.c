 ...
 */
 ...
 /* <usermanual>
@keyword COPPER COLOR

@english

The ''COPPER COLOR'' command is a fundamental instruction used 
in copper lists to dynamically change the color of the any color register, 
...
When a ''COPPER COLOR'' instruction is encountered in the copper 
list, the processor directly writes the color value to the hardware 
color register of the target graphics chip (which will vary 
...
Il comando ''COPPER COLOR'' è un'istruzione fondamentale 
utilizzata nelle copper list per modificare dinamicamente il valore
di un registro di colore dello schermo, laddove sia presente. La copper list è una 
...
Quando un'istruzione ''COPPER COLOR'' viene incontrata nella 
copper list, il processore scrive direttamente il valore di un registro hardware 
del colore del chip grafico di 
...
@syntax    COPPER COLOR RED
@syntax    COPPER WAIT LINE 30
@syntax    COPPER COLOR BLUE
@syntax END COPPER
