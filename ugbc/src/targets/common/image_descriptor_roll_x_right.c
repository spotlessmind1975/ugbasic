...
ImageDescriptor * image_descriptor_roll_x_right( Environment * _environment, ImageDescriptor * _source_image ) {
    ...
    for( y=0; y<copy->height; ++y ) {
        ...
        unsigned char * pixel2g = source + ( y * copy->width * 3 ) + ( (copy->width-1) * 3 ) + 1;
        unsigned char * pixel2b = source + ( y * copy->width * 3 ) + ( (copy->width-1) * 3 ) + 2;        
        ...
        for( x=(copy->width-2); x > -1; --x ) {
            ...
            unsigned char * pixel2r = source + ( y * copy->width * 3 ) + ( (x+1) * 3 );
            unsigned char * pixel2g = source + ( y * copy->width * 3 ) + ( (x+1) * 3 ) + 1;
            unsigned char * pixel2b = source + ( y * copy->width * 3 ) + ( (x+1) * 3 ) + 2;
        ...
        unsigned char * pixel1r = source + ( y * copy->width * 3 );
        unsigned char * pixel1g = source + ( y * copy->width * 3 ) + 1;
        unsigned char * pixel1b = source + ( y * copy->width * 3 ) + 2;
