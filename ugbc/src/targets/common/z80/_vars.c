...
#if defined(__c128z__) || defined(__msx1__) || defined(__coleco__) || defined(__cpc__) || defined(__sc3000__) || defined(__sg1000__) || defined(__vg5000__) || defined(__zx__) || defined(__vz200__)
...
void vars_emit_number( Environment * _environment, char * _name, int _value ) {
    if ( _name ) {
        outline3("%s:  defw $%4.4x,$%4.4x", _name, (unsigned int)( _value & 0xffff ), (unsigned int)( (_value>>16) & 0xffffffff ) );
        if ( _environment->numberConfig.maxBytes > 4 ) {
            out0(" db " );
...
void vars_emit_strips( Environment * _environment, char * _name, Strip * _strips ) {
    ...
    for( i=0; i<maxId; ++i ) {
        ...
        while( actual ) {
            if ( actual->id == i ) {
                outline2("defw %sstrip%d", _name, i );
                break;
            }
            actual = actual->next;
        }
