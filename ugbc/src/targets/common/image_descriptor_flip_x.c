...
ImageDescriptor * image_descriptor_flip_x( Environment * _environment, ImageDescriptor * _source_image ) {
    ...
    for( y=0; y<copy->height; ++y ) {
        for( x=0; x<( copy->width >> 1 ); ++x ) {
            ...
            char * pixel2r = source + ( y * copy->width * copy->depth ) + ( ( copy->width - x - 1 ) * copy->depth );
            char * pixel2g = source + ( y * copy->width * copy->depth ) + ( ( copy->width - x - 1 ) * copy->depth ) + 1;
            char * pixel2b = source + ( y * copy->width * copy->depth ) + ( ( copy->width - x - 1 ) * copy->depth ) + 2;
            char * pixel2a;
            if ( copy->depth > 3 ) {
                pixel2a = source + ( y * copy->width * copy->depth ) + ( ( copy->width - x - 1 ) * copy->depth ) + 3;
            }
