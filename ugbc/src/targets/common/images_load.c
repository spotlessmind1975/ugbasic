...
The ''LOAD IMAGE'' command allows you to load an set of images and to convert each one
into an ''ATLAS''. An Atlas is a set of images, numbered by 0 to frame count, which 
can be used individually with the PUT IMAGE command, referring to the corresponding frame.
The frames are cut out of the original image, using the size provided in the 
''FRAME SIZE (w, h)''. It should be noted that, for this reason, ''ATLAS'' images
...
The resulting ''ATLAS'' can be loaded directly into the expansion
...
Il comando ''LOAD ATLAS'' consente di caricare un set di immagini come ''ATLAS''. 
Un ''ATLAS'' è un set di immagini, numerata per il conteggio di 0 a frame, che
...
L' ''ATLAS'' risultante può essere caricata direttamente nell'espansione
...
@example alienAt11 = LOAD ATLAS("alien.jpg",11) FRAME SIZE AUTO
@example alien2 = LOAD ATLAS("alien.jpg" AS "alien2",11) FRAME SIZE(8,8) TRANSPARENCY
