...
ImageDescriptor * image_descriptor_roll_y_down( Environment * _environment, ImageDescriptor * _source_image ) {
    ...
    for( x=0; x < copy->width; ++x ) {
        ...
        unsigned char * pixel2r = source + ( ( copy->height - 1 ) * copy->width * 3 ) + x * 3;
        unsigned char * pixel2g = source + ( ( copy->height - 1 ) * copy->width * 3 ) + x * 3 + 1;
        unsigned char * pixel2b = source + ( ( copy->height - 1 ) * copy->width * 3 ) + x * 3 + 2;
        ...
        for( y=( copy->height - 2); y > -1; --y ) {
            unsigned char * pixel1r = source + ( y * copy->width * 3 ) + ( x * 3 );
            unsigned char * pixel1g = source + ( y * copy->width * 3 ) + ( x * 3 ) + 1;
            unsigned char * pixel1b = source + ( y * copy->width * 3 ) + ( x * 3 ) + 2;
            unsigned char * pixel2r = source + ( (y+1) * copy->width * 3 ) + ( x * 3 );
            unsigned char * pixel2g = source + ( (y+1) * copy->width * 3 ) + ( x * 3 ) + 1;
            unsigned char * pixel2b = source + ( (y+1) * copy->width * 3 ) + ( x * 3 ) + 2;
        ...
        unsigned char * pixel1r = source + x * 3;
        unsigned char * pixel1g = source + x * 3 + 1;
        unsigned char * pixel1b = source + x * 3 + 2;
