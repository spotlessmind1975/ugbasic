...
/**
 * @brief Emit code for <strong>LEFTB ...</strong>
 * 
...
''LEFTB'' allows the programmer to scroll left a section of the text screen from ''x,y''
with the ''width''  and the ''height'', including the colors, line by line. 
The new lines inserted are filled in the right column with a "blank" character. 
...
''LEFTB'' consente al programmatore di scorrere verso inistra una sezione dello schermo 
di testo da ''x,y'' con la ''width'' e l''height'', inclusi i colori, riga per riga. 
Le nuove righe inserite vengono riempite nella colonna di destra con un carattere "vuoto".
...
@syntax LEFTB y, x, width, height

@example LEFTB 1, 1, 10, 10
