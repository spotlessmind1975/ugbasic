...
void end_storage( Environment * _environment ) {
    ...
    Storage * storageLast = _environment->storage;
    if ( storageLast ) {
        while( storageLast->next ) {
            storageLast = storageLast->next;
        }
        storageLast->next = _environment->currentStorage;
    } else {
        _environment->storage = _environment->currentStorage;
    }

    _environment->currentStorage = NULL;
