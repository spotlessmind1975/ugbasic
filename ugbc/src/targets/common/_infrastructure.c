...
void memory_area_assign( MemoryArea * _first, Variable * _variable ) {
    ...
    while( actual ) {
        ...
        if ( actual->type == MAT_RAM ) {
            if ( _variable->type == VT_STRING || _variable->type == VT_BUFFER || _variable->type == VT_IMAGE || _variable->type == VT_IMAGES || _variable->type == VT_SEQUENCE || _variable->type == VT_MUSIC || _variable->type == VT_SAMPLES || _variable->type == VT_MOVIE ) {
                enoughSpace = 0;
            }
        }
        if ( enoughSpace ) {
...
static Variable * variable_move_from_array_get_address( Environment * _environment, char * _array, char * _field ) {
    ...
    if ( _field ) {
        ...
        field = field_find( array->typeType, _field );
        if ( ! field ) {
...
Variable * variable_temporary( Environment * _environment, VariableType _type, char * _meaning ) {
    ...
    if ( var ) {
        ...
        } else if ( _type == VT_MOVIE ) {
            sprintf(name, "Tmov%d", UNIQUE_ID);
        } else if ( _type == VT_MUSIC ) {
            sprintf(name, "Tmus%d", UNIQUE_ID);
        } else if ( _type == VT_SAMPLES ) {
            sprintf(name, "Tsam%d", UNIQUE_ID);
 ...
 * @return Variable* The source variable
 */
static void variable_increment_decrement_array( Environment * _environment, char * _source, int _direction ) {
    ...
    Variable * source = variable_retrieve( _environment, _source );
    Variable * offset = variable_move_from_array_get_address( _environment, _source, NULL );

    Variable * value = NULL;

    if ( source->bankAssigned == -1 ) {

        value = variable_temporary( _environment, source->arrayType, "(temp)" );

        switch( VT_BITWIDTH( value->type ) ) {
            case 32:
                cpu_move_32bit_indirect2( _environment, offset->realName, value->realName );
                break;
            case 16:
                cpu_move_16bit_indirect2( _environment, offset->realName, value->realName);
                break;
            case 8:
                cpu_move_8bit_indirect2( _environment, offset->realName, value->realName );
                break;
            case 1:
                CRITICAL_DATATYPE_UNSUPPORTED("array(4b)", DATATYPE_AS_STRING[value->type]);
            case 0:
                CRITICAL_DATATYPE_UNSUPPORTED("array(4b)", DATATYPE_AS_STRING[value->type]);
    ...
    if ( source->bankAssigned == -1 ) {
        ...
        switch( VT_BITWIDTH( value->type ) ) {
            ...
            case 1:
            case 0:
                CRITICAL_DATATYPE_UNSUPPORTED("array(3)", DATATYPE_AS_STRING[value->type]);
        }
...
Variable * variable_array_max_vars( Environment * _environment, char * _name ) {
    ...
    if ( array->size > 0 ) {
        ...
        variable_store( _environment, index->name, 0 );
        variable_store( _environment, max->name, VT_MIN( max->type ) );
        Variable * value = variable_temporary( _environment, array->arrayType, "value");
        Variable * startAddress = variable_temporary( _environment, VT_ADDRESS, "(startAddress)");
        cpu_addressof_16bit( _environment, array->realName, startAddress->realName );
        cpu_label( _environment, loopLabel );
...
The ''ARRAY MIN'' statement find out the minimum value of all the elements inside 
a vector or ''array''.

@italian

L'istruzione trova il valore minimo di tutti gli elementi
all'interno di un vettore o matrice array (quindi, un ''array'' 
generico).

@syntax = [ARRAY] MIN( array )

@example IF MIN(vector) = 42 THEN: PRINT "min value is 42": ENDIF
...
Variable * variable_array_min_vars( Environment * _environment, char * _name ) {
    ...
    if ( array->size > 0 ) {
        ...
        min = variable_temporary( _environment, array->arrayType, "(min)");
        variable_store( _environment, index->name, 0 );
        variable_store( _environment, min->name, VT_MAX( min->type ) );
        Variable * value = variable_temporary( _environment, array->arrayType, "value");
        Variable * startAddress = variable_temporary( _environment, VT_ADDRESS, "(startAddress)");
        cpu_addressof_16bit( _environment, array->realName, startAddress->realName );
        cpu_label( _environment, loopLabel );
            ...
            Variable * comparison = variable_greater_than( _environment, min->name, value->name, 0 );
            cpu_compare_and_branch_8bit_const( _environment, comparison->realName, 0, skipLabel, 1 );
            variable_move( _environment, value->name, min->name );
            cpu_label( _environment, skipLabel );
            variable_increment( _environment, index->name );
            variable_compare_and_branch_const( _environment, index->name, sizeInElements, label, 1 );
        cpu_jump( _environment, loopLabel );
...
void variable_move_array1_type_fields( Environment * _environment, char * _array, char * _index, char * _field1, char * _field2 ) {
    ...
    if ( array->bankAssigned == -1 ) {
        ...
        variable_move( _environment, index->name, offset2->name );
        offset2 = variable_sl_const( _environment, offset2->name, VT_OPTIMAL_SHIFT(array->typeType->size) );
        variable_move( _environment, offset2->name, offset1->name );
        cpu_math_add_16bit_const( _environment, offset1->realName, array->absoluteAddress, offset1->realName );
        cpu_math_add_16bit_const( _environment, offset1->realName, field1->offset, offset1->realName );
        cpu_math_add_16bit_const( _environment, offset2->realName, array->absoluteAddress, offset2->realName );
        cpu_math_add_16bit_const( _environment, offset2->realName, field2->offset, offset2->realName );
...
void variable_move_array1_type_const( Environment * _environment, char * _array, char * _index, char * _field, int _value  ) {
    ...
    if ( array->bankAssigned == -1 ) {
        ...
        switch( VT_BITWIDTH( field->type ) ) {
            case 32:
                cpu_poked_const( _environment, offset->realName, _value );
                break;
            case 16:
                cpu_pokew_const( _environment, offset->realName, _value );
                break;
            case 8:
                cpu_poke_const( _environment, offset->realName, _value );
                break;
            case 1:
            case 0:
                CRITICAL_DATATYPE_UNSUPPORTED("array(3)", DATATYPE_AS_STRING[field->type]);
...
static void variable_increment_decrement_array_type( Environment * _environment, char * _source, char * _field, int _direction ) {
    ...
    if ( source->type != VT_TARRAY ) {
        CRITICAL_NOT_ARRAY( _source );
    }
    if ( source->arrayType != VT_TYPE ) {
        CRITICAL_CANNOT_USE_FIELD_ON_NONTYPE( _source );
    }
    if ( !source->typeType ) {
        CRITICAL_CANNOT_USE_FIELD_ON_NONTYPE( _source );
    }
    ...
    if ( source->bankAssigned == -1 ) {
        ...
        value = variable_temporary( _environment, field->type, "(value)" );
        ...
        switch( VT_BITWIDTH( field->type ) ) {
            ...
            case 8:
                ...
                break;
            case 1:
                CRITICAL_DATATYPE_UNSUPPORTED("array(4b)", DATATYPE_AS_STRING[field->type]);
            case 0:
                CRITICAL_DATATYPE_UNSUPPORTED("array(4b)", DATATYPE_AS_STRING[field->type]);
        }
