...
void vars_emit_byte( Environment * _environment, char * _name, int _value ) {
    if ( _name ) {
        outhead2("%s: .byte $%2.2x", _name, (unsigned char)( _value & 0xff ) );
    } else {
...
void vars_emit_word( Environment * _environment, char * _name, int _value ) {
    if ( _name ) {
        outhead2("%s:  .word $%4.4x", _name, (unsigned int)( _value & 0xffff ) );
    } else {
...
void vars_emit_dword( Environment * _environment, char * _name, int _value ) {
    if ( _name ) {
        outhead2("%s:  .dword $%8.8x", _name, (unsigned int)( _value & 0xffffffff ) );
    } else {
...
void vars_emit_strips( Environment * _environment, char * _name, Strip * _strips ) {
    ...
    for( i=0; i<maxId; ++i ) {
        ...
        while( actual ) {
            if ( actual->id == i ) {
                outline2(".word %sstrip%d", _name, i );
                break;
            }
            actual = actual->next;
        }
