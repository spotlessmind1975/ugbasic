...
void flash( Environment * _environment, char * _index, char * _register ) {
    ...
    gtia_flash_begin( _environment, idx->realName, reg!=NULL?reg->realName:NULL );
    ...
    while( i < _environment->flashVarsIndex ) {
        ...
        ++i;
        gtia_flash_register( _environment, idx->realName, timer->realName, color->realName );
    }

    gtia_flash_end( _environment );
