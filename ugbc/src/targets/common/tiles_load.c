...
@english
The ''TILES LOAD'' command allows you to load an image and to convert it into
a set of ''TILES''.
...
@italian
Il comando ''TILES LOAD'' permette di caricare un file immagine, e di convertirlo
...
@syntax = TILES LOAD([filename])

@example starship = TILES LOAD("starship.png")
