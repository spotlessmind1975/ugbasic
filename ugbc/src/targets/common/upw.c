...
/**
 * @brief Emit code for <strong>UPB ...</strong>
 * 
...
''UPW'' allows the programmer to scroll up a section of the text screen from ''x,y''
with the ''width'' and the ''height'', including the colors, line by line. The new 
lines inserted are filled in the bottom line with the correspondent character
...
''UPW'' consente al programmatore di scorrere verso l'alto una sezione dello schermo 
di testo da ''x,y'' con la ''width'' e l''height'', inclusi i colori, riga per riga. 
Le nuove righe inserite vengono riempite nella riga inferiore con il carattere che
esce dal bordo inferiore.
...
@syntax UPW y, x, width, height

@example UPW 1, 1, 10, 10
...
void upw( Environment * _environment, char * _line, char * _column, char * _width, char * _height ) {
    ...
    text_vscroll_screen( _environment, -1, 1 );
