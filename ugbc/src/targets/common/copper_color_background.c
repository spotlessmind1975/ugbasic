 ...
 */
 ...
 /* <usermanual>
@keyword COPPER COLOR BACKGROUND

@english

The ''COPPER COLOR BACKGROUND'' command is a fundamental instruction used 
in copper lists to dynamically change the color of the screen background, 
...
When a ''COPPER COLOR BACKGROUND'' instruction is encountered in the copper 
list, the processor directly writes the color value to the hardware 
background color register of the target graphics chip (which will vary 
...
Il comando ''COPPER COLOR BACKGROUND'' è un'istruzione fondamentale 
utilizzata nelle copper list per modificare dinamicamente il colore 
del sfondo dello schermo, laddove sia presente. La copper list è una 
...
Quando un'istruzione ''COPPER COLOR BACKGROUND'' viene incontrata nella 
copper list, il processore scrive direttamente il valore del colore 
nel registro hardware del colore del sfondo del chip grafico di 
...
@syntax    COPPER COLOR BACKGROUND RED
@syntax    COPPER WAIT LINE 30
@syntax    COPPER COLOR BACKGROUND BLUE
@syntax END COPPER
...
void copper_color_background( Environment * _environment, int _color ) {
    ...
    color->operation = COP_COLOR_BACKGROUND;
    color->param1 = _color;
