...
/**
 * @brief Emit code for <strong>LOAD SAMPLES(...)</strong>
 * 
 * @param _environment Current calling environment
 * @param _filename Filename to read into buffer
 */
/* <usermanual>
@keyword LOAD SAMPLES

@english

The ''LOAD SAMPLES'' command allows you to load a file with audio samples from the file system 
of the host system and use it, as audio samples, directly in the program. Loading occurs 
...
@syntax = LOAD SAMPLES( filename )
@syntax = LOAD SAMPLES( filename AS alias )

@example fugue = LOAD SAMPLES("fugue.wav")
@example fugueAnotherCopy = LOAD SAMPLES("fugue.wav" as "fugue2")
...
Variable * samples_load( Environment * _environment, char * _filename, char * _alias, int _bank_expansion ) {
    ...
    if ( _environment->tenLinerRulesEnforced ) {
        CRITICAL_10_LINE_RULES_ENFORCED( "LOAD SAMPLES");
    }

    if ( _environment->sandbox ) {
        CRITICAL_SANDBOX_ENFORCED( "LOAD SAMPLES");
    }
