...
void put_image( Environment * _environment, char * _image, char * _x, char * _y, char * _frame, int _flags ) {
    ...
    switch( image->type ) {
        case VT_SEQUENCE:
            if ( !frame ) {
                vic1_put_image( _environment, image->realName, x->realName, y->realName, "", image->frameSize, _flags );
            } else {
                vic1_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, image->frameSize, _flags );
            }
            break;
        case VT_IMAGES:
            if ( !frame ) {
            ...
            break;
        case VT_IMAGE:
            vic1_put_image( _environment, image->realName, x->realName, y->realName, NULL, 0, _flags );
            break;
        default:
