...
Variable * random_value( Environment * _environment, VariableType _type ) {
    ...
    Variable * seed = variable_retrieve( _environment, "CPURANDOM_INTERNALSEED" );

    Variable * result = variable_temporary( _environment, _type, "(random value)" );

    switch( VT_BITWIDTH( _type ) ) {
        case 8:
            cpu6502_random_8bit( _environment, "$9004", result->realName );
            break;
        case 16:
            cpu6502_random_16bit( _environment, "$9004", result->realName );
            break;
        case 32:
            cpu6502_random_32bit( _environment, "$9004", result->realName );
            break;
    }
