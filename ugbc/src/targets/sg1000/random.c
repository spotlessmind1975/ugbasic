...
Variable * random_value( Environment * _environment, VariableType _type ) {
    ...
    switch(_type) {
        case VT_BYTE:
        case VT_COLOR:
            z80_random_8bit( _environment, "SG1000TIMER", result->realName );
            break;
        case VT_WORD:
        case VT_POSITION:
        case VT_ADDRESS:
            z80_random_16bit( _environment, "SG1000TIMER", result->realName );
            break;
        case VT_DWORD:
            z80_random_32bit( _environment, "SG1000TIMER", result->realName );
            break;
    }
