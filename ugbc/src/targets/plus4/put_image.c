...
void put_image( Environment * _environment, char * _image, char * _x, char * _y, char * _frame ) {
    ...
    switch( image->type ) {
        case VT_IMAGES:
            if ( !frame ) {
                ted_put_image( _environment, image->realName, x->realName, y->realName, "", image->frameSize );
            } else {
                ted_put_image( _environment, image->realName, x->realName, y->realName, frame->realName, image->frameSize );
            }
            break;
        case VT_IMAGE:
            ted_put_image( _environment, image->realName, x->realName, y->realName, NULL, 0 );
            break;
        default:
