    ...
    switch( resource->type ) {
        case VT_SEQUENCE:
            if ( image->bankAssigned != -1 ) {
                ...
                if ( !sequence ) {
                    if ( !frame ) {
                        gime_flip_image( _environment, resource, "", "", image->frameSize, image->frameCount, _direction );
                    } else {
                        gime_flip_image( _environment, resource, frame->realName, "", image->frameSize, image->frameCount, _direction );
                    }
                } else {
                    if ( !frame ) {
                        gime_flip_image( _environment, resource, "", sequence->realName, image->frameSize, image->frameCount, _direction );
                    } else {
                        gime_flip_image( _environment, resource, frame->realName, sequence->realName, image->frameSize, image->frameCount, _direction );
                    }
                }
            }
            break;
        case VT_IMAGES:
            if ( image->bankAssigned != -1 ) {
                ...
                if ( !frame ) {
                    ...
                    gime_flip_image( _environment, resource, frame->realName, NULL, image->frameSize, 0, _direction );
                }
            }
            break;
        case VT_IMAGE:
