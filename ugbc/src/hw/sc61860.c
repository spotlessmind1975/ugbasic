...
void sc61860_math_mul_8bit_to_16bit( Environment * _environment, char *_source, char *_destination,  char *_other, int _signed ) {
    ...
    // inline( cpu_math_mul_8bit_to_16bit )
        ...
        if ( _signed ) {
            ...
            // outline0("XOR 0xFF" );
            // outline0("INC A" );
            // outline1("JMP %s// done1", label );

            // outhead1("%spositive:", label );
            // outline1("LD A, (%s)", _source );

            // outhead1("%s// done1:", label );
            // outline0("LD H, A" );
...
void sc61860_math_div2_const_8bit( Environment * _environment, char *_source, int _steps, int _signed ) {
    ...
    // inline( cpu_math_div2_const_8bit )
        ...
        if ( _signed ) {
            ...
            sc61860_complement2_16bit( _environment, _source, _source );
...
void sc61860_compare_16bit( Environment * _environment, char *_source, char *_destination,  char *_other, int _positive ) {
    ...
    // embedded( cpu_compare_16bit, src_hw_sc61860_cpu_compare_16bit_asm )
        ...
        // outline1("LD IX, 0x%4.4x", ( (0xff*_positive) << 8 ) | ( 0xff*(1-_positive)) );
...
void sc61860_compare_16bit_const( Environment * _environment, char *_source, int _destination,  char *_other, int _positive ) {
    ...
    // inline( cpu_compare_16bit )
        ...
        // outline1("LD A, (%s)", _source);
        // outline0("LD B, A");
        // outline1("LD A, 0x2.2x", (unsigned char)(_destination&0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "1"));
        // outline0("LD B, A");
        // outline1("LD A, 0x2.2x", (unsigned char)((_destination>>8)&0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, 0x%2.2x", 0xff*_positive);
        // outline1("LD (%s), A", _other);
    ...
    // embedded( cpu_compare_16bit, src_hw_sc61860_cpu_compare_16bit_asm )
        ...
        // outline1("LD HL, %s", _source);
        // outline1("LD DE, 0x%4.4x", _destination);
        // outline1("LD IX, 0x%4.4x", ( (0xff*_positive) << 8 ) | ( 0xff*(1-_positive)) );
...
void sc61860_compare_and_branch_16bit_const( Environment * _environment, char *_source, int _destination,  char *_label, int _positive ) {
    ...
    // inline( cpu_compare_and_branch_16bit_const )
        ...
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "1"));
        // outline1("CP 0x%2.2x", ( _destination >> 8 ) & 0xff );
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", _source);
        // outline1("CP 0x%2.2x", ( _destination & 0xff ) );
        // outline1("JP NZ, %s", label);
        if ( _positive ) {
            // outline1("JP %s", _label);
            // outhead1("%s:", label );
...
void sc61860_less_than_16bit_const( Environment * _environment, char *_source, int _destination,  char *_other, int _equal, int _signed ) {
    ...
    // inline( cpu_less_than_16bit )
        ...
        if ( _signed ) {
            ...
            // outline1("LD HL, 0x%4.4x", ( _destination & 0xffff ) );
            // outline1("LD DE, (%s)", _source);
            // outline0("LD A, H" );
            // outline0("XOR D" );
            // outline1("JP M,%scmpgte2", label );
...
void sc61860_math_mul_16bit_to_32bit( Environment * _environment, char *_source, char *_destination,  char *_other, int _signed ) {
    ...
    // embedded( cpu_math_mul_16bit_to_32bit, src_hw_sc61860_cpu_math_mul_16bit_to_32bit_asm );
 ...
 * @param _steps Times to halves
 */
void sc61860_math_div2_const_16bit( Environment * _environment, char *_source, int _steps, int _signed ) {
...
void sc61860_store_32bit( Environment * _environment, char *_destination, int _value ) {
    ...
    // inline( cpu_store_32bit )
        ...
        // outline1("LD (%s), HL", _destination );
        // outline1("LD HL, 0x%2.2x", ( ( _value >> 16 ) & 0xffff ) );
...
void sc61860_compare_32bit_const( Environment * _environment, char *_source, int _destination,  char *_other, int _positive ) {
    ...
    // inline( cpu_compare_32bit )
        ...
        // outline1("LD A, (%s)", _source);
        // outline0("LD B, A");
        // outline1("LD A, 0x%2.2x", (unsigned char)(_destination & 0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "1"));
        // outline0("LD B, A");
        // outline1("LD A, 0x%2.2x", (unsigned char)((_destination>>8) & 0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "2"));
        // outline0("LD B, A");
        // outline1("LD A, 0x%2.2x", (unsigned char)((_destination>>16) & 0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "3"));
        // outline0("LD B, A");
        // outline1("LD A, 0x%2.2x", (unsigned char)((_destination>>24) & 0xff));
        // outline0("CP B");
        // outline1("JP NZ, %s", label);
        // outline1("LD A, 0x%2.2x", 0xff*_positive);
        // outline1("LD (%s), A", _other);
    ...
    // embedded( cpu_compare_32bit, src_hw_sc61860_cpu_compare_32bit_asm )
        ...
        // outline1("LD HL, %s", _source);
        // outline1("LD DE, 0x%4.4x", (unsigned int)(_destination&0xffff));
        // outline1("LD IY, 0x%4.4x", (unsigned int)((_destination>>16)&0xffff));
        // outline1("LD IX, 0x%4.4x", ( (0xff*_positive) << 8 ) | ( 0xff*(1-_positive)) );
...
void sc61860_compare_and_branch_32bit_const( Environment * _environment, char *_source, int _destination,  char *_label, int _positive ) {
    ...
    // inline( cpu_compare_and_branch_32bit_const )
        ...
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "3"));
        // outline1("CP 0x%2.2x", ( _destination >> 24 ) & 0xff );
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "2"));
        // outline1("CP 0x%2.2x", ( _destination >> 16 ) & 0xff );
        // outline1("JP NZ, %s", label);
        // outline1("LD A, (%s)", address_displacement(_environment, _source, "1"));
        // outline1("CP 0x%2.2x", ( _destination >> 8 ) & 0xff );
        // outline1("JP NZ, %s", label);
