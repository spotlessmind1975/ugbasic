...
void vic2_text_at( Environment * _environment, char * _x, char * _y, char * _text, char * _size ) {
    ...
    Variable * text_address = variable_retrieve( _environment, "text_address" );

    vic2_get_width( _environment, "$24" );

    outline1("LDA %s", _text );
    outline0("STA $20");
    outline1("LDA %s+1", _text );
    outline0("STA $21");
    outline1("LDA %s", text_address->realName );
    outline0("STA $22");
    outline1("LDA %s+1", text_address->realName );
    outline0("STA $23");
    outline1("LDX %s", _y );
    ...
    outline0("CMP #32");
    outline1("BCC %sp128", label );
    outline0("CMP #64");
    outline1("BCC %sp0", label );
    outline0("CMP #96");
    outline1("BCC %sm64", label );
    outline0("CMP #160");
    outline1("BCC %sp64", label );
    outline0("CMP #192");
    outline1("BCC %sm64", label );
    outline0("CMP #224");
    outline1("BCC %sm128", label );
    outline1("JMP %sp0", label );
    outhead1("%sp64:", label );
    outline0("ADC #64");
    outline1("JMP %sp0", label );
    outhead1("%sp128:", label );
    outline0("ADC #128");
    outline1("JMP %sp0", label );
    outhead1("%sm64:", label );
    outline0("SBC #63");
    outline1("JMP %sp0", label );
    outhead1("%sm128:", label );
    outline0("SBC #127");
    outline1("JMP %sp0", label );
    outhead1("%sp0:", label );

    // outline0("CMP #32");
    // outline1("BEQ %snoconv", label );
    // outline0("CMP #33" );
    // outline1("BEQ %snoconv", label );
    // outline0("CMP #42" );
    // outline1("BEQ %snoconv", label );
    // outline0("CMP #48" );
    // outline1("BCS %snumconv", label );
    // outhead1("%sconv:", label );
    // outline0("SEC" );
    // outline0("SBC #$40" );
    outhead1("%snoconv:", label );
    outline0("STA ($22),Y");
    outline0("INY" );
    outline0("DEX" );
    outline1("BNE %sloop2", label );
    outline1("JMP %send", label );
    // outhead1("%snumconv:", label );
    // outline0("CMP #58" );
    // outline1("BCC %snoconv", label );
    // outline1("JMP %sconv", label );
    outhead1("%send:", label );
}
