...
FORMATSTR_COPY_DIGITS:
	CP (HL)
	JR Z, FORMATSTR_COPY_DIGITSZ
	JR FORMATSTR_COPY_DIGITSNZ
FORMATSTR_COPY_DIGITSZ:
	PUSH AF
	LD A, 0
	LD (IXLR), A
	POP AF
	JR FORMATSTR_COPY_DIGITSDONE
FORMATSTR_COPY_DIGITSNZ:
	PUSH AF
	LD A, 0xff
	LD (IXLR), A
	POP AF
	JR FORMATSTR_COPY_DIGITSDONE
FORMATSTR_COPY_DIGITSDONE:
...
FORMATSTR_PAD_RIGHT2:
	LD (HL), '0'
	CALL REPLACEMENT_CPI
	JR Z, FORMATSTR_PAD_RIGHT2Z
	JR FORMATSTR_PAD_RIGHT2NZ
FORMATSTR_PAD_RIGHT2Z:
	PUSH AF
	LD A, 0
	LD (IXLR), A
	POP AF
	JR FORMATSTR_PAD_RIGHTDONE
FORMATSTR_PAD_RIGHT2NZ:
	PUSH AF
	LD A, 0xff
	LD (IXLR), A
	POP AF
	JR FORMATSTR_PAD_RIGHTDONE
FORMATSTR_PAD_RIGHTDONE:
	; P/V is reset in case of overflow (if BC=0 after calling LDI).
	PUSH BC
	PUSH AF
	LD A, B
