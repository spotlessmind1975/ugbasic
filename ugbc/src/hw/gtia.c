...
static void calculate_frame_buffer( Environment * _environment, int _size_required ) {
    ...
    int start = ( (_environment->frameBufferStart) >> 8 ) & 0x01;
    int end = ( (_environment->frameBufferStart + _size_required) >> 8 ) & 0x01;
    if ( start != end ) {
        _environment->frameBufferStart = ( _environment->frameBufferStart >> 13 ) << 13;
    }
}
...
static Variable * gtia_image_converter_bitmap_mode_standard( Environment * _environment, char * _source, int _width, int _height, int _depth, int _offset_x, int _offset_y, int _frame_width, int _frame_height, int _transparent_color, int _flags ) {
    ...
    *(buffer+2) = _frame_height;
    ...
    if ( lastUsedSlotInCommonPalette > 1 ) {
        *(buffer + 3 + ( ( _frame_width >> 3 ) * _frame_height ) + 1 ) = commonPalette[1].index;
    } else {
        *(buffer + 3 + ( ( _frame_width >> 3 ) * _frame_height ) + 1 ) = 0xff;
    }

    if ( lastUsedSlotInCommonPalette > 0 ) {
        *(buffer + 3 + ( ( _frame_width >> 3 ) * _frame_height ) ) = commonPalette[0].index;
    } else {
...
void gtia_image_compile_multicolor_mode_standard( Environment * _environment, ParamsImageCompile * _params ) {
...
void gtia_image_compile( Environment * _environment, ParamsImageCompile * _params ) {
...
void gtia_images_compile_multicolor_mode_standard( Environment * _environment, ParamsImagesCompile * _params ) {
...
void gtia_images_compile_bitmap_mode_standard( Environment * _environment, ParamsImagesCompile * _params ) {
...
void gtia_images_compile( Environment * _environment, ParamsImagesCompile * _params ) {
    ...
    switch( _mode ) {
        ...
        // register encoded in the bit pattern to plot the color.
        case BITMAP_MODE_ANTIC8:
            return gtia_images_compile_multicolor_mode_standard( _environment, _params );
        ...
        // memory is needed for a display of similiar-sized pixels.
        case BITMAP_MODE_ANTIC9:
            return gtia_images_compile_bitmap_mode_standard( _environment, _params );
...
void gtia_sequence_compile_multicolor_mode_standard( Environment * _environment, ParamsSequenceCompile * _params ) {
...
void gtia_sequence_compile_bitmap_mode_standard( Environment * _environment, ParamsSequenceCompile * _params ) {
...
void gtia_sequence_compile( Environment * _environment, ParamsSequenceCompile * _params ) {
    ...
    switch( _mode ) {
        ...
        // register encoded in the bit pattern to plot the color.
        case BITMAP_MODE_ANTIC8:
            return gtia_sequence_compile_multicolor_mode_standard( _environment, _params );
        ...
        // memory is needed for a display of similiar-sized pixels.
        case BITMAP_MODE_ANTIC9:
            return gtia_sequence_compile_bitmap_mode_standard( _environment, _params );
...
void gtia_put_image( Environment * _environment, Resource * _image, char * _x, char * _y, char * _frame, char * _sequence, int _frame_size, int _frame_count, char * _flags ) {
    ...
    if ( _image->isCompiled ) {
        ...
        deploy( putimagecompiled, src_hw_gtia_put_image_compiled_asm );
