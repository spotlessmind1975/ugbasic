...
/**
 * @brief <i>GTIA</i>: emit code to set raster irq
 * 
...
/**
 * @brief <i>GTIA</i>: emit code to wait for next raster irq
 * 
...
/**
 * @brief <i>GTIA</i>: emit code to wait for next raster irq at different position
 * 
 * This function outputs assembly code needed to wait for the
...
static int gtia_screen_mode_enable( Environment * _environment, ScreenMode * _screen_mode ) {
    ...
    switch( _screen_mode->id ) {
        ...
        case BITMAP_MODE_ANTIC10:
            ...
            // 112
            DLI_BLANK( dliListCurrent, 8 );
            // 72	\Display ANTIC mode 10 (BASIC 5) 64+10
            // 64	|Screen memory starts at
            // 156	/64+156*256 =40000
            DLI_LMS( dliListCurrent, 10, 40000 );
