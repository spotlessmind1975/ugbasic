...
typedef enum _OutputFileType {
    ...
    OUTPUT_FILE_TYPE_DSK_OLD = 17,
    OUTPUT_FILE_TYPE_DSK_NEW = 18
...
typedef struct _VestigialConfig {
    ...
    char rchack_4gravity_1181; // up to 1.18.1
...
#define DOJO_PUT_MESSAGE_ARRAY_TYPE_UNSUPPORTED( v, t ) CRITICAL3("E368 - type of array's elements is not supported", v, t );
#define DOJO_PUT_MESSAGE_ARRAY_SIZE_UNSUPPORTED( v, t ) CRITICAL3("E369 - array is too large to send as message", v, t );
#define DOJO_GET_MESSAGE_MISSING_VARIABLE( ) CRITICAL("E370 - missing variable for GET MESSAGE" );
#define DOJO_PUT_MESSAGE_MISSING_VARIABLE( ) CRITICAL("E371 - PUT MESSAGE needs a DOJOKA variable" );
#define CRITICAL_VECTOR_GET_X_VECTOR_NEEDED( v ) CRITICAL2("E372 - X needs a VECTOR variable", v );
#define CRITICAL_VECTOR_GET_Y_VECTOR_NEEDED( v ) CRITICAL2("E373 - Y needs a VECTOR variable", v );
#define CRITICAL_FILE_NOT_FOUND( n ) CRITICAL2("E374 - file not found", n );
#define CRITICAL_TYPE_NESTED_UNSUPPORTED( n ) CRITICAL2("E375 - cannot define a nested TYPE (a TYPE inside a TYPE)", n ); 
#define CRITICAL_TYPE_ALREADY_DEFINED( n ) CRITICAL2("E376 - TYPE already defined", n ); 
#define CRITICAL_TYPE_NOT_OPENED( ) CRITICAL("E377 - cannot END an unopened TYPE" ); 
#define CRITICAL_CANNOT_DEFINE_OUTSIDE_TYPE( n ) CRITICAL2("E378 - cannot define outside a TYPE", n ); 
#define CRITICAL_CANNOT_USE_DATATYPE_IN_TYPE( n ) CRITICAL2("E379 - cannot use this type inside a TYPE", n );  
#define CRITICAL_VARIABLE_TYPE_NEEDED( n ) CRITICAL2("E380 - variable TYPE is needed", n );  
#define CRITICAL_UNKNOWN_TYPE( n ) CRITICAL2("E381 - unknown TYPE", n );  
#define CRITICAL_CANNOT_USE_FIELD_ON_NONTYPE( n ) CRITICAL2("E382 - cannot access to fields of a non TYPE variable", n );  
#define CRITICAL_UNKNOWN_FIELD_ON_TYPE( n ) CRITICAL2("E383 - unknown TYPE field ", n );  
#define CRITICAL_DATA_NOT_ENOUGH_FOR_TYPE( n ) CRITICAL2("E384 - not enough value on DATA for given TYPE ", n );  
#define CRITICAL_FILE_CSV_NOT_ENOUGH_DATA( n ) CRITICAL2("E385 - not enough data in CSV to fillup the array ", n );  
#define CRITICAL_UNCLOSED_EMBEDDED_ESCAPE_SEQUENCE( n ) CRITICAL2("E386 - unclosed embedded conditional", n );  
#define CRITICAL_CANNOT_PUT_TILEMAP_FOR_TILEMAP_ON_STORAGE( n ) CRITICAL2("E387 - cannot use (yet) PUT TILEMAP on tilemap on storage", n );  
#define CRITICAL_CANNOT_ACCESS_MULTITHREAD_ARRAY_OUTSIDE_PROCEDURE( n ) CRITICAL2("E388 - cannot use multithread array access operator outside PARALLEL PROCEDURE", n );   
#define CRITICAL_NESTED_COPPER_LIST_NOT_ALLOWED( ) CRITICAL("E389 - cannot define nested COPPER list" );   
#define CRITICAL_COPPER_LIST_NOT_OPENED( ) CRITICAL("E390 - COPPER list is not opened" );   
#define CRITICAL_COPPER_LIST_ALREADY_DEFINED( ) CRITICAL("E391 - COPPER list already defined" );   
#define CRITICAL_MOVE_WITH_NOT_ALLOWED_TYPE( t ) CRITICAL2("E392 - cannot MOVE this data type inside a COPPER list", t );   
#define CRITICAL_STORE_WITH_NOT_ALLOWED_TYPE( t ) CRITICAL2("E393 - cannot STORE this data type inside a COPPER list", t );   
#define CRITICAL_STORE_WITH_NOT_CONST_NOT_ALLOWED( t ) CRITICAL2("E394 - cannot STORE using not const in a COPPER list", t );
#define CRITICAL_COLOR_WITH_NOT_CONST_NOT_ALLOWED( t ) CRITICAL2("E395 - cannot COLOR using not const in a COPPER list", t );
#define CRITICAL_WAIT_INVALID_VALUE( t ) CRITICAL2i("E396 - invalid value for WAIT LINE inside the COPPER list", t );
#define CRITICAL_COPPER_LIST_UNKNOWN( n ) CRITICAL2("E397 - unknown COPPER list using COPPER USE", n );
#define CRITICAL_INVALID_NUMBER_BYTES( n ) CRITICAL2i("E398 - invalid number of bytes for NUMBER representation", n );
