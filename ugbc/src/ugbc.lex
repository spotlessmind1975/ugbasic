...
[a-f][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9]* { yylval.string = strdup(yytext); RETURN(BufferDefinitionHex,1); }
...
Bm { RETURN(BITMAP,1); }
BITMAPADDRESS { RETURN(BITMAPADDRESS,1); }
...
COMPILE { RETURN(COMPILE,1); }
COMPILED { RETURN(COMPILED,1); }
COm { RETURN(COMPILED,1); }
COMPLETE { RETURN(COMPLETE,1); }
CONNECTED { RETURN(CONNECTED,1); }
Con { RETURN(CONNECTED,1); }
COS { RETURN(COS,1); }
COPPER { RETURN(COPPER,1); }
Cop { RETURN(COPPER,1); }
Cx { RETURN(COS,1); }
...
CPU6510 { RETURN(CPU6510,1); }
CPU6309 { RETURN(CPU6309,1); }
CPU6809 { RETURN(CPU6809,1); }
CPU7501 { RETURN(CPU7501,1); }
CPU8086 { RETURN(CPU8086,1); }
CPU8501 { RETURN(CPU8501,1); }
...
F7 { RETURN(F7,1); }
F8 { RETURN(F8,1); }
FCIRCLE { RETURN(FCIRCLE,1); }
Fc { RETURN(FCIRCLE,1); }
FELLIPSE { RETURN(FELLIPSE,1); }
Fe { RETURN(FELLIPSE,1); }
FILE { RETURN(FILEX,1); }
FILL { RETURN(FILL,1); }
Fil { RETURN(FILL,1); }
FIRE { RETURN(FIRE,1); }
...
S { RETURN(S,1); }
SAFE { RETURN(SAFE,1); }
SAMPLES { RETURN(SAMPLES,1); }
Sam { RETURN(SAMPLES,1); }
Sa { RETURN(SAFE,1); }
SAVE { RETURN(SAVE,1); }
Sv { RETURN(SAVE,1); }
...
TRIANGLE { RETURN(TRIANGLE,1); }
Trg { RETURN(TRIANGLE,1); }
TRUE { RETURN(TRUE,1); }
Tr { RETURN(TRUE,1); }
TSB { RETURN(TSB,1); }
...
\"(\\.|[^"\\])*\" { yylval.string = strdup(yytext); memmove(yylval.string,yylval.string+1,strlen(yylval.string)); yylval.string[strlen(yylval.string)-1]=0; RETURN(String,1);  }
#\"(\\.|[^"\\])*\" { yylval.string = strdup(yytext); memmove(yylval.string,yylval.string+2,strlen(yylval.string)-2); yylval.string[strlen(yylval.string)-3]=0; RETURN(RawString,1);  }
